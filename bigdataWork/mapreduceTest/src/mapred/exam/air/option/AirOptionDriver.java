 ...
 * 2. run메서드를 오버라이딩
 * 		=> run메서드 안에 Driver에서 구현했던 모든 코드를 구현
 * 3. run메서드를 main메서드에서 실행되도록 호출해야 한다.
 * 		=> run메서드를 직접 호출할 수 없고 ToolRunner라는 헬퍼 클래스를 이용해서 호출
*/
public class AirOptionDriver extends Configured implements Tool {
	
	@Override
	public int run(String[] optionList) throws Exception {
		//run메서드는 사용자가 입력한 모든 옵션에 대한 정보를 String[]로 전달받는다.
