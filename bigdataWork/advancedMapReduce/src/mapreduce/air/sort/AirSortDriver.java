...
public class AirSortDriver extends Configured implements Tool {
	...
	public int run(String[] optionList) throws Exception {
		...
		Job job = new Job(getConf(), "air_sort");	// job이름은 그냥 표시되는 이름이지만 어떤 job인지 구분하기 위한 이름이기 때문에 바꾸는 게 좋음
		
		job.setMapperClass(AirSortMapper.class);	// 어떤 클래스를 먼저 정의하든지 순서는 상관없음
